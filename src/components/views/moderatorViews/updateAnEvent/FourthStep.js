   ...
   // const id = useSelector(state => state.event.id)
    ...
    return (
        <section>
            ...
            <p>{addressEvent}</p>
            <p>{categoryList}</p>
            <button className="save-changeButton" type="button" onClick={() =>
                dispatch(updateInfoEvent(id,nameEvent, typeEvent, shortDescription, longDescription, addressEvent, cityEvent, startDate, startTime,endTime, organizersName, imgUrl,categoryList))}
